...
//매 프레임 마다 lenis와 scrolltrigger를 같이 업데이트 해야 스크롤 애니메이션이 어긋나지 않음
function raf(time) {
    //지금 프레임의 시간을 lenis에게 알려준다
    lenis.raf(time);
    //ScrollTrigger가 현대 스크롤 위치에 다시 계산하게 한다
...
function updateActiveFromViewport() {
    ...
    sectionMap.forEach((id) => {
        ...
        //섹션 안이면 거리가 0, 밖이면 위아래 경계중 더 가까운 거리로 계산한다
...
//페이지가 처음 열렸을때는 hero 메뉴를 active로 시작한다
...
//메뉴를 누르면 브라우저 기본 점프이동을 막고 lenis로 부드럽게 이동
navLinks.forEach((link) => {
...
gsap.timeline({ default: { duration: 0.8, ease: "power2.out" } })
    ...
    //설명 등장을 제목보다 살짝 늦게 나오게 한다 : (<0.12) 사용
...
function splitWords(selector, wordClass) {
    ...
    document.querySelectorAll(selector).forEach((textEl) => {
        //textEl 요소 안의 텍스트를 가져와서 앞 뒤 공백을 제거한 뒤에 공백 기준으로 나누어 배열로 만드는 코드
...
//설명문장도 motion_word의 단어 span으로 나눔
...
//카드 위에 마우스를 올리면 빛이 따라다니는 것 처럼 보이는 레이어를 만든다
document.querySelectorAll('.feature_card').forEach((card) => {
    const glass = document.createElement('div');
    glass.className = 'glass_reflect'; //앞에 className이 있어서 .찍으면 안됨~!!
...
document.querySelectorAll('.tit_card').forEach((card) => {
    ...
    card.addEventListener('mousemove', (e) => {
        const rect = card.getBoundingClientRect();
        //카드 안에서 마우스가 가로,세로로 몇 퍼센트 위치인지 계산
        const px = (e.clientX - rect.left) / rect.width;
        const py = (e.clientY - rect.top) / rect.height;

        //위 아래 위치를 retateX 각도로 바꾼다
        const rx = (py - 0.5) * -15;
        const ry = (px - 0.5) * 22;
...
const pinTl = gsap.timeline({
    scrollTrigger: {
        trigger: '.pin_scene',
        start: 'top top',
        end: '+=1800', //1800px 스크롤 하는 동안 타임라인이 진행된다
        pin: true, //섹션을 화면에 고정해서 사진이 올라오는 무대를 만든다
        scrub: true, //스크롤 위치와 애니메이션 진행도를 연결한다
        anticipatePin: 1, // pin이 시작될 때 생길 수 있는 튐을 줄인다
        invalidateOnRefresh: true, // 새로고침이나 리사이즈 때 값을 다시 계산한다
        toggleActions: 'play none none reset', // 내려갈 때만 재생하고, 다시 위에서 진입하면 초기화한다
...
//사진마다 살짝 다른 각도와 giltch 효과를 줘서 디자인 캘러리 처럼 겹쳐보이게 한다
photos.forEach((photo, index) => {
    ...
    pinTl.add(() => {
        ...
        //0.4초 뒤에 glitch 클래스를 제거하여 효과를 끝낸다
    ...
    pinTl.fromTo(photo, {
        ...
        rotate: (index % 2) ? 5 : -5,
        duration: 0.85,
        ease: 'power3.out', //빠르게 올라온 뒤 부드럽게 멈추게 한다
    }, index * 0.22)
...
//트랙의 전체 너비에서 화면에 이미 보이는 너비를 빼면 이동 거리만 남음
function totalWidth() {
